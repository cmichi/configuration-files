...
#       in zsh-help() below.
# precmd() => this is run before the prompt is displayed
is4 && [[ $NOPRECMD -eq 0 ]] && precmd () {
    [[ $NOPRECMD -gt 0 ]] && return 0
    # update VCS information
    vcs_info

    if [[ -n "$(ps -p $PPID | grep '.*sshd$')" ]] ; then
    #if [[ -n "$(pstree -p | grep ".*sshd.*$PPID.*$$.*")" ]] ; then
       NAME="$USER@$HOST:"
    fi
    ...
    case $TERM in
        ...
        # set title
            if [[ -n ${VCS_INFO_message_1_} ]] ; then
                ...
                ESC_print "$NAME""Shell""$ROOT_IDENTIFY"
            fi
        ;;
        xterm*|rxvt)
        print -Pn "\e]0;%n@%m: %~\a"
...
preexec () {
    ...
    case $TERM in
      screen*)
        ...
        ESC_print "$NAME$CMD$ROOT_IDENTIFY"
      ;;
      xterm*|rxvt)
        set_title "${(%):-"%n@%m:"}" "$1"
      ;;
...
${BLUE}%n${NO_COLOUR}@%m %40<...<%B%~%b%<< "'${VCS_INFO_message_0_}'"%# "
        ...
        # This assembles the primary prompt string RED=$'%{\e[31m%}'
        if (( EUID != 0 )); then
            if [[ $TERM == screen* ]] ; then
                PROMPT="${NO_COLOUR}\$ ${RED}${EXITCODE}${WHITE}${debian_chroot:+($debian_chroot)}"'${VCS_INFO_message_0_}'"${GREEN}>${NO_COLOUR} "
            else
                PROMPT="${RED}${EXITCODE}${WHITE}${debian_chroot:+($debian_chroot)}${GREEN}%n${NO_COLOUR}@${BLUE}%m${YELLOW} %40<...<%B%~%b%<< "'${VCS_INFO_message_0_}'"%# "
            fi

        else
            if [[ $TERM == screen* ]] ; then
                PROMPT="${NO_COLOUR}# ${BLUE}${EXITCODE}${WHITE}${debian_chroot:+($debian_chroot)}"'${VCS_INFO_message_0_}'"${RED}>${NO_COLOUR} "
            else
                PROMPT="${BLUE}${EXITCODE}${WHITE}${debian_chroot:+($debian_chroot)}${RED}%n${NO_COLOUR}@${BLUE}%m${YELLOW} %40<...<%B%~%b%<< "'${VCS_INFO_message_0_}'"%# "
            fi
        fi
    fi
fi
...
# shortcut aliases
alias univpn='cd /etc/openvpn/ && sudo openvpn univpn.conf'
alias vi='vim'
alias cls='clear'
alias rst='reset'
alias top='htop'
alias clean='rm -f *~; rm -f .*~; rm -f *.class'
alias hms='sudo netcfg -a; sudo netcfg hms'
alias uu='sudo netcfg -a; sudo netcfg uu'
alias vnc='sudo x11vnc -ncache -display :0 -rfbauth ~/.x11vnc/passwd -auth /var/run/slim.auth'
alias browse='wminput 00:17:AB:39:0E:85 -c /etc/cwiid/wminput/browse'
...
alias fplay='vlc -I ncurses /tmp/Flash*'
...
wiki () { article=`echo $@ | sed -e 's/ /_/g'`; dig +short txt $article.wp.dg.cx; }
